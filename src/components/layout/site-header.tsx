  ...
  return (
    <div className={`h-[var(--header-height)] ${spacerBackgroundClass}`}>
      <header className={`fixed inset-x-0 top-0 z-50 h-[var(--header-height)] bg-black text-white transition-opacity duration-220 ease-out ${isHidden ? "pointer-events-none opacity-0" : "opacity-100"}`}>
        <PageContainer className="site-header__container flex h-full items-center justify-between gap-[clamp(0.5rem,2.35vw,2rem)]">
            ...
            return (
              ...
              >
                ...
                {!isActive && (
                  <Image
                    alt=""
                    className="header-nav-link__decoration pointer-events-none absolute bottom-0 left-1/2 z-0 h-[var(--header-nav-decoration-height)] w-[var(--header-nav-decoration-width)]"
                    height={47.889}
                    src={item.hoverAsset}
                    width={72.929}
                  />
        ...
        <button
          ...
          className="mobile-menu-trigger hidden size-[var(--header-menu-size)] shrink-0 max-[1350px]:flex"
      ...
      <button
        aria-label="메뉴 닫기"
        className="mobile-menu-backdrop fixed inset-x-0 bottom-0 top-[var(--header-height)] z-30 hidden max-[1350px]:block"
        data-open={isMenuOpen}
